        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-lg font-semibold md:text-xl leading-none tracking-tight">
                    {{ isset($generalinformation) ? 'Edit General Information' : 'Create General Information' }}
                </h1>
            </div>
        </div>
        <form
            action="{{ isset($generalinformation) ? route('generalinformation.update', $generalinformation->id) : route('generalinformation.store') }}"
            ...
            <div class="flex flex-wrap -mx-2 mb-4 border-b border-gray-200">
                <div class="w-full md:w-1/2 px-2 mb-4">
                    <div class="p-4">
                        ...
                        <p class="text-sm text-gray-600 mb-4">Provide the basic details of the general information.</p>
                        <div class="flex flex-col w-full mb-4 border-b gap-4 border-gray-200">
                            <div class="mb-4 w-full">
                                <label for="name" class="block text-sm font-medium text-gray-700">Name</label>
                                <input type="text" name="name" id="name"
                                    value="{{ old('name', $generalinformation->name ?? '') }}"
                            ...
                            <div class="mb-4 w-full">
                                ...
                                <input type="text" name="title" id="title"
                                    value="{{ old('title', $generalinformation->title ?? '') }}"
                                    class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
                                @error('title')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                                ...
                                <select name="city_id" id="city_id"
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ (old('city_id') == $city->id || (isset($generalinformation) && $generalinformation->city_id == $city->id)) ? 'selected' : '' }}>
                                            {{ $city->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <!-- <div class="p-4 border-b border-gray-200 rounded-md">
                        ...
                        <div class="mb-4">
                            <image-uploader name="image" id="image"
                                v-bind:initial-file="'{{ old('image', $generalinformation->image ?? '') }}'"
                                class="mt-1 block rounded-md border-gray-200 shadow-sm py-1"></image-uploader>
                        </div>
                        <div class="mb-4">
                            <label for="image_alt" class="block text-sm font-medium text-gray-700">Featured Image Alt Text</label>
                            <input type="text" name="image_alt" id="image_alt"
                                value="{{ old('image_alt', $generalinformation->image_alt ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
                        </div>
                    </div> -->
                </div>
            ...
            <!-- Missing Content Repeater Section -->
            <div class="mb-4 p-4 border-b border-gray-200">
                <h2 class="text-md font-semibold mb-2">Content</h2>
                <content-repeater :initial-data="{{ json_encode(old('repeater_content', $generalinformation->repeater_content ?? [])) }}"
            ...
            <div class="mb-4">
                <button type="submit" :disabled="isUploading"
                    class="bg-black text-white p-2 rounded disabled:opacity-50">{{ isset($generalinformation) ? 'Update General Information' : 'Save General Information' }}</button>
            </div>
        </form>
    </div>
</x-app-layout>
