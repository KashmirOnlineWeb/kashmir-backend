<x-app-layout>
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-lg font-semibold md:text-xl leading-none tracking-tight">Things to do</h1>
            <p class="text-sm text-gray-500">Manage your Things to do items here.</p>
        </div>
        <a href="{{ route('thingstodo.create') }}"
                    ...
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div id="app-{{ $item->id }}" class="relative inline-block text-left">
                                <dropdown-menu :edit-url="'{{ route('thingstodo.edit', $item->id) }}'"
                                    :delete-url="'{{ route('thingstodo.destroy', $item->id) }}'"></dropdown-menu>
                            </div>
                        </td>
                    </tr>
                @endforeach
    ...
    <div class="mt-4">
        {{ $thingstodo->links('components.pagination') }}
    </div>
</x-app-layout>
