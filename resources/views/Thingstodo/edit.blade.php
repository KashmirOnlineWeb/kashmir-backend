        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-lg font-semibold md:text-xl leading-none tracking-tight">
                    {{ isset($thingstodo) ? 'Edit Things to do' : 'Create Things to do' }}
                </h1>
            </div>
        </div>
        <form
            action="{{ isset($thingstodo) ? route('thingstodo.update', $thingstodo->id) : route('thingstodo.store') }}"
                        ...
                        <div class="flex flex-col w-full mb-4 border-b gap-4 border-gray-200">
                            ...
                            <div class="mb-4 w-full">
                                ...
                                <input type="text" name="title" id="title"
                                    value="{{ old('title', $thingstodo->title ?? '') }}"
                                    class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
                                @error('title')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                                ...
                                <select name="city_id" id="city_id"
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ (old('city_id') == $city->id || (isset($thingstodo) && $thingstodo->city_id == $city->id)) ? 'selected' : '' }}>
                                            {{ $city->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="p-4 border-b border-gray-200 rounded-md">
                        ...
                        <div class="mb-4">
                            <image-uploader name="image" id="image"
                                v-bind:initial-file="'{{ old('image', $thingstodo->image ?? '') }}'"
            ...
            <div class="mb-4 p-4 border-b border-gray-200">
                ...
                <content-repeater :initial-data="{{ json_encode(old('repeater_content', $thingstodo->repeater_content ?? [])) }}"
            ...
            <div class="mb-4">
                <button type="submit" :disabled="isUploading"
                    class="bg-black text-white p-2 rounded disabled:opacity-50">{{ isset($thingstodo) ? 'Update Things to do' : 'Save Things to do' }}</button>
            </div>
        </form>
    </div>
</x-app-layout>
