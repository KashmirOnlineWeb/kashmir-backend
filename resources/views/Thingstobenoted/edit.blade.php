        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-lg font-semibold md:text-xl leading-none tracking-tight">
                    {{ isset($thingstobenoted) ? 'Edit Things to be Noted' : 'Create Things to be Noted' }}
                </h1>
            </div>
        </div>
        <form
            action="{{ isset($thingstobenoted) ? route('thingstobenoted.update', $thingstobenoted->id) : route('thingstobenoted.store') }}"
            method="POST" enctype="multipart/form-data">
            @csrf
            @if (isset($thingstobenoted))
                @method('PUT')
            @endif
                        ...
                        <div class="flex flex-col w-full mb-4 border-b gap-4 border-gray-200">
                            <div class="mb-4 w-full">
                                ...
                                <input type="text" name="name" id="name"
                                    value="{{ old('name', $thingstobenoted->name ?? '') }}"
                            ...
                            <div class="mb-4 w-full">
                                ...
                                <input type="text" name="title" id="title"
                                    value="{{ old('title', $thingstobenoted->title ?? '') }}"
                                    class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
                                @error('title')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                                ...
                                <select name="city_id" id="city_id"
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ (old('city_id') == $city->id || (isset($thingstobenoted) && $thingstobenoted->city_id == $city->id)) ? 'selected' : '' }}>
                                            {{ $city->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <div class="p-4 border-b border-gray-200 rounded-md">
                        ...
                        <div class="mb-4">
                            <image-uploader name="image" id="image"
                                v-bind:initial-file="'{{ old('image', $thingstobenoted->image ?? '') }}'"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="image_alt" id="image_alt"
                                value="{{ old('image_alt', $thingstobenoted->image_alt ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
                        </div>
                    </div>
                </div>
            ...
            <div class="mb-4 p-4 border-b border-gray-200">
                ...
                <content-repeater :initial-data="{{ json_encode(old('repeater_content', $thingstobenoted->repeater_content ?? [])) }}"
            ...
            <div class="mb-4">
                <button type="submit" :disabled="isUploading"
                    class="bg-black text-white p-2 rounded disabled:opacity-50">{{ isset($thingstobenoted) ? 'Update Things to be Noted' : 'Save Things to be Noted' }}</button>
            </div>
        </form>
    </div>
</x-app-layout>
