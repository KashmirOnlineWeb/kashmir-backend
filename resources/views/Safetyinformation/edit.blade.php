        ...
        <div class="flex items-center justify-between mb-4">
            <div>
                <h1 class="text-lg font-semibold md:text-xl leading-none tracking-tight">
                    {{ isset($safetyinformation) ? 'Edit Safety Information' : 'Create Safety Information' }}
                </h1>
            </div>
        </div>
        <form
            action="{{ isset($safetyinformation) ? route('safetyinformation.update', $safetyinformation->id) : route('safetyinformation.store') }}"
            method="POST" enctype="multipart/form-data">
            @csrf
            @if (isset($safetyinformation))
                @method('PUT')
            @endif
            ...
            <div class="flex flex-wrap -mx-2 mb-4 border-b border-gray-200">
                        ...
                        <div class="flex flex-col w-full mb-4 border-b gap-4 border-gray-200">
                            <div class="mb-4 w-full">
                                ...
                                <input type="text" name="name" id="name"
                                    value="{{ old('name', $safetyinformation->name ?? '') }}"
                            ...
                            <div class="mb-4 w-full">
                                ...
                                <input type="text" name="title" id="title"
                                    value="{{ old('title', $safetyinformation->title ?? '') }}"
                                    class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
                                @error('title')
                                    <span class="text-red-500 text-sm">{{ $message }}</span>
                                @enderror
                                ...
                                <select name="city_id" id="city_id"
                                    ...
                                    @foreach ($cities as $city)
                                        <option value="{{ $city->id }}"
                                            {{ (old('city_id') == $city->id || (isset($safetyinformation) && $safetyinformation->city_id == $city->id)) ? 'selected' : '' }}>
                                            {{ $city->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                    ...
                    <!-- <div class="p-4 border-b border-gray-200 rounded-md">
                        ...
                        <div class="mb-4">
                            <image-uploader name="image" id="image"
                                v-bind:initial-file="'{{ old('image', $safetyinformation->image ?? '') }}'"
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="image_alt" id="image_alt"
                                value="{{ old('image_alt', $safetyinformation->image_alt ?? '') }}"
                                class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
                        </div>
                    </div> -->
                </div>
             ...
             <div class="mb-4 p-4 border-b border-gray-200">
                <h2 class="text-md font-semibold mb-2">Safety Numbers</h2>
                <safety-info :initial-data="{{ json_encode(old('description', $safetyinformation->description ?? [])) }}"
            ...
            <div class="mb-4 p-4 border-b border-gray-200">
                ...
                <content-repeater :initial-data="{{ json_encode(old('repeater_content', $safetyinformation->repeater_content ?? [])) }}"
            ...
            <div class="mb-4">
                <button type="submit" :disabled="isUploading"
                    class="bg-black text-white p-2 rounded disabled:opacity-50">{{ isset($safetyinformation) ? 'Update Safety Information' : 'Save Safety Information' }}</button>
            </div>
        </form>
    </div>
</x-app-layout>
