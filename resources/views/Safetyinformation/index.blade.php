<x-app-layout>
    <div class="flex items-center justify-between mb-4">
        <div>
            <h1 class="text-lg font-semibold md:text-xl leading-none tracking-tight">Safety Information</h1>
            <p class="text-sm text-gray-500">Manage your safety information items here.</p>
        </div>
        <a href="{{ route('safetyinformation.create') }}"
    ...
    <div class="rounded-lg border shadow-sm">
        <table class="min-w-full divide-y divide-gray-200">
            ...
            <tbody class="bg-white divide-y divide-gray-200">
                @foreach ($safetyinformation as $item)
                    <tr>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div id="app-{{ $item->id }}" class="relative inline-block text-left">
                                <dropdown-menu :edit-url="'{{ route('safetyinformation.edit', $item->id) }}'"
                                    :delete-url="'{{ route('safetyinformation.destroy', $item->id) }}'"></dropdown-menu>
                            </div>
                        </td>
                    </tr>
                @endforeach
    ...
    <div class="mt-4">
        {{ $safetyinformation->links('components.pagination') }}
    </div>
</x-app-layout>
