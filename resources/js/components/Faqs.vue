      ...
      <div v-for="(info, index) in safetyInfos" :key="info.id" class="relative mb-4 p-4 border rounded-md flex items-start py-4">
        ...
        <div class="flex-grow pl-4 w-full sm:w-auto">
          ...
          <div class="w-full mb-4">
            <label class="block text-sm font-medium text-gray-700">Question</label> <!-- {{ edit_1 }} -->
            <input required type="text" v-model="info.question" :name="`${namePrefix}[${index}][question]`" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
          </div>
          <!-- Answer -->
          <div class="mb-4">
            <label class="block text-sm font-medium text-gray-700">Answer</label> <!-- {{ edit_2 }} -->
            <input required type="text" v-model="info.answer" :name="`${namePrefix}[${index}][answer]`" class="mt-1 block w-full rounded-md border-gray-200 shadow-sm py-1">
          </div>
        </div>
      </div>
...
const props = defineProps({
    ...
    namePrefix: { // {{ edit_3 }}
        type: String,
        default: 'faqInfo' // Updated name prefix for FAQs
    }
});
...
if (Array.isArray(initialData) && initialData.length > 0) {
    safetyInfos.value = initialData.map(item => ({
        id: idCounter++,
        question: item.question || '', // {{ edit_4 }}
        answer: item.answer || '' // {{ edit_5 }}
    }));
} else {
    safetyInfos.value.push({ id: idCounter++, question: '', answer: '' }); // Default entry
}

const addInfo = () => {
    safetyInfos.value.push({ id: idCounter++, question: '', answer: '' }); // {{ edit_6 }}
};
